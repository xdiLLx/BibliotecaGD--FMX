...
uses
  ...
  FMX.Objects,
  FMX.MultiResBitmap,
  FMX.Platform, DelphiZXIngQRCode, GD.Types;
...
type
  ...
  published
    ...
    property ForeGround: TAlphaColor read FForeGround write SetForeGround
      default TAlphaColors.Black;
    property IconBitmap: TFixedMultiResBitmap read GetIconBitmap
      write SetIconBitmap;
...
const
  TFMXPlatforms = pidWin32 or pidWin64 or pidOSX32 or pidiOSSimulator or
...
begin
  ...
  begin
    if not Assigned(FQRImage) then
      FQRImage := TBitmap.Create(Round(Scale1 * Self.Width),
        Round(Scale1 * Self.Height));
...
function TGDQRCode.GetVersion: string;
begin
...
begin
  inherited;
  CreateQRImage;
  Canvas.DrawBitmap(FQRImage, FQRImage.Bounds, RectF(0, 0, Width, Height),
