...
uses
  ...
  GD.Styles, FMX.Forms, System.Types;
...
function GetCurrentMonitorIndex(Form: TForm): Integer;
...
{$ENDREGION}
    property OnChange: TNotifyChange read FOnChange write FOnChange;
    property ActiveFontColor: TAlphaColor read FActiveFontColor write SetActiveFontColor;
  end;
...
var
  I: Integer;
  Display: TDisplay;
  FormCenterPoint: TPointF;
begin
  Result := -1; // Default to -1, indicating not found
  if Form <> nil then
  begin
    // Encontrar o ponto central do formulário
    FormCenterPoint := PointF(Form.Left + Form.Width / 2, Form.Top + Form.Height / 2);

    // Iterar pelos monitores para encontrar aquele que contém o ponto central do formulário
    for I := 0 to Screen.DisplayCount - 1 do
    begin
      Display := Screen.Displays[I];
      if (FormCenterPoint.X >= Display.WorkArea.Left) and (FormCenterPoint.X <= Display.WorkArea.Right) and
         (FormCenterPoint.Y >= Display.WorkArea.Top) and (FormCenterPoint.Y <= Display.WorkArea.Bottom) then
      begin
        Result := I; // Monitor encontrado
        Break;
      end;
    end;
  end;
